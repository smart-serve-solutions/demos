...
(function (w) {
  ...
  const S = {
    ...
    locId: "L1",
    term: 1,
    vendedor: "Kevin Solano",
    offline: false,
    queue: 3,
    cart: { cliId: null, lineas: [] },
    posSel: null,
    catSel: "A1",
    catQ: "",
    catTab: "Producto",
  ...
  function openLocalPop() {
    ...
    const fila = (
      l,
    ) => `<button class="pop-item" data-loc="${l.id}" aria-current="${l.id === S.locId}">${icon("pin")}
    ...
    U.popover(
      ...
      `<div class="pop-h">Tiendas</div>${D.tiendas.map(fila).join("")}
       <div class="pop-h">Centro y bodegas</div>${D.locales
         .filter((l) => l.tipo !== "tienda")
         .map(fila)
         .join("")}`,
      (root) =>
        $$("[data-loc]", root).forEach((b) =>
  ...
  function toggleEnlace() {
    ...
    if (S.offline) {
      toast(
        "Enlace caído",
        "El local sigue operando contra su nodo. Los documentos se encolan y salen al reconectar.",
        "wa",
      );
      qTimer = setInterval(() => {
        S.queue++;
  ...
  /* la opción del menú donde está el usuario (la misma de las migas de pan) */
  const aqui = () =>
    S.screen && S.screen !== "inicio" ? N.menuPathFor(S.screen) : null;
  function modDeAqui() {
    const p = aqui();
    return p ? p.mod.id : null;
  }

  function openMenu() {
    menuState.open = true;
    $("#overlayRoot").innerHTML = `<div class="menu-scrim" id="menuScrim"></div>
    ...
    /* el menú arranca justo debajo de la barra superior, mida lo que mida
       (en el celular la barra ocupa dos filas y tapaba el buscador) */
    const tb = $("#topbar");
    if (tb) {
      const alto = tb.offsetHeight + "px";
      $("#menuScrim").style.top = alto;
      $(".menu-panel").style.top = alto;
    }
    $("#menuScrim").addEventListener("click", closeMenu);
    document.addEventListener("keydown", escCloseMenu);
    renderMenuGroups();
    renderMenuRight();
    /* el módulo activo también queda a la vista (lista larga en pantallas
       bajas, franja horizontal en el celular); se mide con el pie ya puesto */
    const activo = $('#menuGroups .mg[aria-current="true"]');
    if (activo) {
      const vs = document.scrollingElement
        ? document.scrollingElement.scrollTop
        : 0;
      activo.scrollIntoView({ block: "nearest", inline: "nearest" });
      if (document.scrollingElement) document.scrollingElement.scrollTop = vs;
    }
    const mq = $("#menuQ");
    mq.value = menuState.q;
    mq.focus();
    mq.setSelectionRange(mq.value.length, mq.value.length);
    mq.addEventListener("input", () => {
      menuState.q = mq.value;
      updateActiveModuleButtons();
      renderMenuRight();
    });
    const b = $("#btnMenu");
  ...
  function renderMenuGroups() {
    ...
    wrap.innerHTML =
      ...
      N.MODULES.map(
        (m) =>
          `<button class="mg" data-mod="${m.id}" aria-current="${!menuState.q && menuState.module === m.id}">${icon(m.ic)}<span>${esc(m.t)}</span><span class="cnt" title="Requerimientos de la matriz en este módulo">${N.menuReqCount(m.id)}</span></button>`,
      ).join("");
    wrap
  ...
  function updateActiveModuleButtons() {
    ...
    $$(".mg[data-mod]", wrap).forEach((b) =>
      b.setAttribute(
        "aria-current",
        String(!menuState.q && b.dataset.mod === menuState.module),
      ),
    );
  }
  ...
  function buscarDatos(q) {
    ...
    D.articulos.forEach((a) => {
      if (out.length < 8 && hit(a.cod + " " + a.desc + " " + a.marca))
        out.push({
          ic: a.tipo === "Servicio" ? "wrench" : "box",
          t: a.desc,
          k: a.cod + " · " + c(a.precio),
          go: () => {
            S.catSel = a.id;
    ...
    D.clientes.forEach((x) => {
      if (out.length < 14 && hit(x.nom + " " + x.ced))
        out.push({
          ic: "users",
          t: x.nom,
          k: x.ced,
    ...
    D.proveedores.forEach((x) => {
      if (out.length < 18 && hit(x.nom + " " + x.ced))
        out.push({
          ic: "truck",
          t: x.nom,
          k: x.ced,
  ...
  function renderMenuRight() {
    ...
    const curMod =
      N.MODULES.find((m) => m.id === menuState.module) || N.MODULES[0];
    ...
    const ridLabel = (it) =>
      (it.reqs || []).length <= 1
        ? esc(it.reqs[0] || "")
        : esc(it.reqs[0]) + " +" + (it.reqs.length - 1);
    const ridTitle = (it) => esc((it.reqs || []).join(", "));
    const chevIc = icon(
      "chev",
      'style="width:14px;height:14px;color:var(--ink-4)"',
    );
    ...
    function leafRow(it) {
      ...
      const on = !!(actual && it === actual.item);
      ...
      return `<button class="msi"${on ? ' aria-current="page" title="Aquí está"' : ""} ${it.action ? `data-action="${it.action}"` : `data-goid="${it.screen}"${argAttr(it)}`}>${inner}</button>`;
    }
    function searchRow(it) {
      const clickable = !!(it.screen || it.action);
      const attr = it.action
        ? `data-action="${it.action}"`
        : it.screen
          ? `data-goid="${it.screen}"${argAttr(it)}`
          : `data-jump="${it.modId}"`;
    ...
    if (buscando) {
      html =
        ...
        (results.length
          ...
          : datos.length
            ? ""
            : `<div class="mut" style="padding:34px 6px;text-align:center;font-size:14px">Sin resultados para «${esc(menuState.q)}». Pruebe con otro término.</div>`);
    } else {
      html = `<div class="menu-modh"><span class="mit" style="width:32px;height:32px">${icon(curMod.ic)}</span>
        ...
        <div class="msec-list">${curSections
          .map(
            (sec) =>
              `<div class="msec${actual && sec === actual.sec ? " aqui" : ""}"><div class="msec-h"><span class="mit-sm">${icon(sec.ic || curMod.ic)}</span><h4>${esc(sec.t)}</h4></div>
           <div class="msub">${sec.items.map(leafRow).join("")}</div></div>`,
          )
          .join("")}</div>`;
    }
    ...
    itemsEl.scrollTop = 0;
    $("#menuFoot").innerHTML =
      `${icon("info")}<span>${totalReqs} requerimientos de la matriz en ${N.MODULES.length} módulos, agrupados en ${totalItems} opciones de menú · ${totalImpl} con pantalla activa en este demo · escriba para buscar en todos a la vez</span>`;
    /* la opción donde está el usuario queda a la vista (se mide después
       del pie, que también ocupa alto): si no cabe en la primera pantalla,
       su sección sube arriba; si aún no se ve, se centra */
    const marcada =
      !buscando && itemsEl.querySelector('.msi[aria-current="page"]');
    if (marcada) {
      const seVe = () => {
        const a = marcada.getBoundingClientRect(),
          b = itemsEl.getBoundingClientRect();
      ...
      if (!seVe()) {
        const vs = document.scrollingElement
          ? document.scrollingElement.scrollTop
          : 0;
        marcada.closest(".msec").scrollIntoView({ block: "start" });
        if (!seVe()) marcada.scrollIntoView({ block: "center" });
        if (document.scrollingElement) document.scrollingElement.scrollTop = vs;
      }
    }
    ...
    $$("[data-dato]", itemsEl).forEach((b) =>
      b.addEventListener("click", () => {
        closeMenu();
        datos[+b.dataset.dato].go();
      }),
    );
    $$("[data-action]", itemsEl).forEach((b) =>
      b.addEventListener("click", () => {
    ...
    $$("[data-jump]", itemsEl).forEach((b) =>
      b.addEventListener("click", () => {
        menuState.module = b.dataset.jump;
        menuState.q = "";
        $("#menuQ").value = "";
        updateActiveModuleButtons();
        renderMenuRight();
  ...
  APP.workspace = function (id, cfg) {
    ...
    function barra() {
      return `<div class="tabs" role="tablist" aria-label="${esc(cfg.title)}">${cfg.tabs
        .map((t) => {
          const on = t.id === st.tab;
          const b = t.badge ? t.badge() : null;
          return `<button type="button" class="tab" role="tab" id="${tabId(t.id)}" data-tab="${t.id}"
          aria-selected="${on}" aria-controls="tp-${id}" tabindex="${on ? 0 : -1}"><span>${esc(t.t)}</span>${
            b && b.n
              ? `<span class="tb ${b.k || ""}" title="${esc(b.l || "")}">${b.n}</span>`
              : ""
          }</button>`;
        })
        .join("")}</div>`;
    ...
    APP.screen(id, {
      ...
      prep(arg) {
        ...
        const s = String(arg),
          i = s.indexOf(":");
        const tab = i < 0 ? s : s.slice(0, i),
          dato = i < 0 ? null : s.slice(i + 1);
      ...
      render(v) {
        v.innerHTML =
          barra() +
          `<div role="tabpanel" id="tp-${id}" aria-labelledby="${tabId(st.tab)}"></div>`;
        cur().render($("#tp-" + id, v));
      },
      wire(v) {
        ...
        const ir = (t, foco) => {
          ...
          render();
          if (foco) {
            const n = document.getElementById(tabId(t));
            if (n) n.focus();
          }
        };
        tabs.forEach((b, i) => {
          b.addEventListener("click", () => {
            if (b.dataset.tab !== st.tab) ir(b.dataset.tab);
          });
  ...
  function render() {
    ...
    if (def.html) {
      v.innerHTML = def.bare
        ? def.html()
        : `<div class="${def.pad || "pad"}">${cabeza}${def.html()}</div>`;
    } else {
      v.innerHTML = def.bare
        ? ""
  ...
  function setTema(t) {
    ...
    try {
      localStorage.setItem("sc-tema", t);
    } catch (e) {}
    render();
  }
  APP.tema = {
    es: esOscuro,
    set: setTema,
    alterna: () => setTema(esOscuro() ? "light" : "dark"),
  };
  ...
  APP.start = function () {
    ...
    let t = "light";
    try {
      t = localStorage.getItem("sc-tema") || "light";
    } catch (e) {}
    ...
    document.addEventListener("mouseover", (e) => {
      ...
      const r = U.rectZ(b);
      tip.style.left =
        Math.max(8, Math.min(r.left, U.anchoZ() - tip.offsetWidth - 8)) + "px";
